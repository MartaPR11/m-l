...
class UsuarioController
{
    ...
    public function entrar(){
        ...
        //Si ha pulsado el botón de acceder, tramito el formulario
            ...
            $campo_clave = filter_input(INPUT_POST, "paw", FILTER_SANITIZE_STRING);
    ...
    public function editar($id){
        ...
        if (isset($_POST["guardar"])){
            ...
            $paw = filter_input(INPUT_POST, "paw", FILTER_SANITIZE_STRING);
            $usuarios = (filter_input(INPUT_POST, 'usuarios', FILTER_SANITIZE_STRING) == 'on') ? 1 : 0;
            $personajes=filter_input(INPUT_POST, "personajes", FILTER_SANITIZE_STRING);
            $cambiar_clave = (filter_input(INPUT_POST, 'cambiar_clave', FILTER_SANITIZE_STRING) == 'on') ? 1 : 0;

            //Encripto la clave
            $clave_encriptada = ($paw) ? password_hash($paw,  PASSWORD_BCRYPT, ['cost'=>12]) : "";

            if ($id == "nuevo"){

                //Creo un nuevo usuario
                $this->db->exec("INSERT INTO usuarios (usuario, paw, personajes,usuarios ) VALUES ('$usuario','$clave_encriptada','$personajes','$usuarios')");
                ...
                ($cambiar_clave) ?
                    $this->db->exec("UPDATE usuarios SET usuario='$usuario',paw='$clave_encriptada',personajes='$personajes',='$usuarios' WHERE id='$id'") :
                    $this->db->exec("UPDATE usuarios SET usuario='$usuario',personajes='$personajes',usuarios='$usuarios' WHERE id='$id'");
